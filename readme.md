...
Use the reproduction scripts instead of copying individual training, rendering, and evaluation commands. Activate `dynamic-2dgs` first, then run one scene at a time:

```bash
DNERF_SCENES=(lego bouncingballs jumpingjacks hook mutant standup trex hellwarrior)
for scene in "${DNERF_SCENES[@]}"; do
  script/reproduce_dnerf.sh "$scene"
done
```
...
for scene in "${DGMESH_SCENES[@]}"; do
  script/reproduce_dgmesh.sh "$scene"
done
```

`reproduce_dnerf.sh` trains, renders, and evaluates the D-NeRF scene. `reproduce_dgmesh.sh` additionally arranges the official DG-Mesh evaluator inputs and runs CD/EMD evaluation. The DG-Mesh evaluator repository must be available at `$HOME/DG-Mesh`, and its Python environment must be active; override these defaults with `DGMESH_REPO_ROOT`, `DGMESH_EVAL_ROOT`, and `DGMESH_EVAL_PYTHON` when needed. Set `FORCE=1` to rerun completed DG-Mesh stages. See [`script/reproduce_dnerf.sh`](script/reproduce_dnerf.sh) and [`script/reproduce_dgmesh.sh`](script/reproduce_dgmesh.sh) for all supported environment variables and checks.
